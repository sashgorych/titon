...
( function ( $ ) {
    ...
    $.fn.listnav = function ( options ) {
        ...
        return this.each( function () {
            ...
            function init() {
                $wrapper.prepend( createLettersHtml() );
            ...
            function addClasses() {
                ...
                $( $list ).children().each( function () {
                    $this = $( this ), firstChar = '', str = $.trim( $this.children(":first").text() ).toLowerCase();
                    if ( str !== '' ) {
                        if ( hasPrefixes ) {
                            spl = str.split( ' ' );
            ...
            function createLettersHtml() {
                ...
                for ( var i = 0; i < letters.length; i++ ) {
                    if ( html.length === 0 ) {
                        html.push( '<li><a class="ln-all" role="tab" aria-controls="panel-all" aria-selected="false" href="#">'+window.cmtt_listnav_data.listnav.allLabel+'</a></li>' );
                        if ( opts.includeNums ) {
                            html.push( '<li><a class="ln-_" tabindex="-1" role="tab" aria-controls="panel-_" aria-selected="false" href="#">0-9</a></li>' );
                        }
                    }
                    if ( letters[i] === '_' )
                    {
                    ...
                    html.push( '<li><a class="ln-' + ( letters[i] === '-' ? '-' : i ) + ' lnletter-' + letters[i].toLowerCase() + '" tabindex="-1" role="tab" aria-selected="false" aria-controls="panel-' + letters[i].toLowerCase() + ' href="#" >' + ( ( letters[i] === '-' ) ? '...' : letters[i].toUpperCase() ) + '</a></li>' );

                }
                return '<ul class="glossary-abc ln-letters">' + html.join( '' ) + '</ul>' + ( ( opts.showCounts ) ? '<div class="ln-letter-count" style="display:none; position:absolute; top:0; left:0; min-width:20px;">0</div>' : '' );
            }
            init();
        } );
    };
    $.fn.listnav.defaults = { initLetter: '', includeAll: true, incudeOther: false, includeNums: true, flagDisabled: true, noMatchText: 'No matching entries', showCounts: false, cookieName: null, onClick: null, prefixes: [ ], perPage: 0, letter: [ 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z' ] };
} )( jQuery );
